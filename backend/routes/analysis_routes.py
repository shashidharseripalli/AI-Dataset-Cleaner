...
from backend.services.visualization_service import build_visualization_payload
...
async def run_visualization(payload: AnalysisRequest):
    csv_file = _normalize_csv_path(payload.csv_path)
    if not csv_file.exists():
        raise HTTPException(status_code=404, detail="CSV file not found")
    if csv_file.suffix.lower() != ".csv":
    ...
    try:
        return await run_in_threadpool(build_visualization_payload, str(csv_file))
    except Exception as exc:
